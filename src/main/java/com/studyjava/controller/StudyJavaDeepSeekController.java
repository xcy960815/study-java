...
public class StudyJavaDeepSeekController {
  ...
  @Resource private StudyJavaDeepSeekService studyJavaDeepSeekService;
  ...
  public ResponseBodyEmitter completions(
      @Valid @RequestBody StudyJavaDeepSeekCompletionsRequestDto completionsRequestDto) {
    // 设置超时时间，例如 5 分钟
  ...
  public StudyJavaDeepSeekModelsVo models() {
    try {
      ...
      throw new StudyJavaException(
          e.getMessage() != null
              ? e.getMessage()
              : "DeepSeek API Error: " + e.getClass().getSimpleName());
    }
  }
  ...
  public StudyJavaDeepSeekBalanceVo balance() {
    try {
      ...
      for (StudyJavaDeepSeekBalanceDto.BalanceInfo balanceInfoDto : balanceDto.getBalance_infos()) {
        StudyJavaDeepSeekBalanceVo.BalanceInfo balanceInfoVo =
            new StudyJavaDeepSeekBalanceVo.BalanceInfo();
