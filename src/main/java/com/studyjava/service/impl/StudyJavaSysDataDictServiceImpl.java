 ...
 */
@Service
public class StudyJavaSysDataDictServiceImpl
    extends ServiceImpl<StudyJavaSysDataDictMapper, StudyJavaSysDataDictDao>
    implements StudyJavaSysDataDictService {
  @Resource private StudyJavaSysDataDictMapper studyJavaSysDataDictMapper;
  ...
  @Override
  public IPage<StudyJavaSysDataDictVo> getDataDictList(
      Page<StudyJavaSysDataDictDto> page, StudyJavaSysDataDictDto studyJavaSysDataDictDto) {

    IPage<StudyJavaSysDataDictDao> daoPage =
        studyJavaSysDataDictMapper.getDataDictList(page, dto2Dao(studyJavaSysDataDictDto));
    // 转换为VO对象
    IPage<StudyJavaSysDataDictVo> voPage =
        new Page<>(daoPage.getCurrent(), daoPage.getSize(), daoPage.getTotal());
    List<StudyJavaSysDataDictVo> voList =
        daoPage.getRecords().stream()
            .map(
                dao -> {
  ...
  public StudyJavaSysDataDictVo getDataDictDetail(Long id) {
    StudyJavaSysDataDictDao studyJavaSysDataDictDao =
        studyJavaSysDataDictMapper.getDataDictDetail(id);
