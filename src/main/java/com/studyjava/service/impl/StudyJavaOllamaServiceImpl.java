...
public class StudyJavaOllamaServiceImpl extends StudyJavaAiService
  ...
  public StudyJavaOllamaModelsDto models() throws IOException, InterruptedException {
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(Ollama_Models_Api)).GET().build();
    // 发送请求并获取响应
  ...
  public StudyJavaOllamaTagsDto tags() throws IOException, InterruptedException {
    // 构建请求
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(Ollama_Tags_Api)).GET().build();
    HttpResponse<String> response =
        httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofString());
  ...
  public StudyJavaOllamaVersionDto version() throws IOException, InterruptedException {
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(Ollama_Version_Api)).GET().build();
    HttpResponse<String> response =
        httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofString());
  ...
  public StudyJavaOllamaPsDto ps() throws IOException, InterruptedException {
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(Ollama_Ps_Api)).GET().build();
    HttpResponse<String> response =
        httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofString());
  ...
  public StudyJavaOllamaShowDto show(StudyJavaOllamaShowVo studyJavaOllamaShowVo)
    ...
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(Ollama_Show_Api))
   ...
   */
  @Override
  public void delete(StudyJavaOllamaDeleteVo studyJavaOllamaDeleteVo)
      throws IOException, InterruptedException {
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(Ollama_Delete_Model_Api))
  ...
  public void completions(
      StudyJavaOllamaCompletionsVo studyJavaOllamaCompletionsVo, SseEmitter emitter) {
    CompletableFuture.runAsync(
        () -> {
          try {
            HttpRequest httpRequest =
                generateRequestBuilder(generateRequestUrl(Ollama_Completions_Api))
  ...
  public void pull(StudyJavaOllamaPullVo studyJavaOllamaPullVo) {
    try {
      HttpRequest httpRequest =
          generateRequestBuilder(generateRequestUrl(Ollama_Pull_Api))
  ...
  /** 处理流式响应 */
  private void handleStreamResponse(HttpResponse<InputStream> response, SseEmitter emitter)
      throws IOException {
