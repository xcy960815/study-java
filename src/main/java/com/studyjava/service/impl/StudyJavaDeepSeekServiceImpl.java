...
public class StudyJavaDeepSeekServiceImpl extends StudyJavaAiService
  ...
  public void completions(
      StudyJavaDeepSeekCompletionsVo studyJavaDeepSeekCompletionsVo, SseEmitter emitter) {
    CompletableFuture.runAsync(
        () -> {
          log.info(
              "开始处理DeepSeek AI对话请求，请求参数：model={}, stream={}, messages.size={}",
              studyJavaDeepSeekCompletionsVo.getModel(),
              studyJavaDeepSeekCompletionsVo.getStream(),
              studyJavaDeepSeekCompletionsVo.getMessages() != null
                  ? studyJavaDeepSeekCompletionsVo.getMessages().size()
                  : 0);
          ...
          try {
            ...
            HttpRequest.BodyPublisher bodyPublisher =
                studyJavaDeepSeekCompletionsVo.getBodyPublisher();
            log.debug("构建请求体完成");

            // 构建HTTP请求
            HttpRequest httpRequest =
                generateRequestBuilder(requestUrl).POST(bodyPublisher).build();
  ...
  public StudyJavaDeepSeekModelsDto models() throws IOException, InterruptedException {
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(DEEPSEEK_MODELS_URL)).GET().build();
  ...
  public StudyJavaDeepSeekBalanceDto balance() throws IOException, InterruptedException {
    HttpRequest httpRequest =
        generateRequestBuilder(generateRequestUrl(DEEPSEEK_BALANCE_URL)).GET().build();
    HttpResponse<String> response =
        httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofString());
