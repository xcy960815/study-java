...
public abstract class StudyJavaAiService {
  ...
  protected <T> T handleResponse(HttpResponse<String> response, Class<T> responseType)
    ...
    if (response.statusCode() == 200) {
      ...
      // 尝试解析错误信息
      try {
        StudyJavaCompletionsErrorDto errorDto =
            objectMapper.readValue(response.body(), StudyJavaCompletionsErrorDto.class);
        throw new StudyJavaAiException(
            errorDto.getError().getCode(), errorDto.getError().getMessage());
      } catch (Exception e) {
  ...
  protected void readResponseLines(HttpResponse<InputStream> response, SseEmitter emitter)
      throws IOException {
    int statusCode = response.statusCode();
    if (statusCode != 200) {
      try {
        StudyJavaCompletionsErrorDto studyJavaCompletionsErrorDto =
            objectMapper.readValue(response.body(), StudyJavaCompletionsErrorDto.class);
        throw new StudyJavaAiException(
            studyJavaCompletionsErrorDto.getError().getCode(),
            studyJavaCompletionsErrorDto.getError().getMessage());
      } catch (Exception e) {
        if (e instanceof StudyJavaAiException) {
          throw (StudyJavaAiException) e;
        }
    ...
    try (BufferedReader reader =
      ...
      while ((line = reader.readLine()) != null) {
        if (!line.trim().isEmpty()
            ...
            && !line.startsWith(KEEPALIVE_FLAG)) {
